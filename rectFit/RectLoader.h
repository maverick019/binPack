...
namespace rbp
{
	/*RectLoader implements the MAXRECTS data structure and different bin packing algorithms that
	use this structure.
	*/
	class RectLoader
	{
	public:
		...
		RectLoader(int width, int height, bool allowFlip = true);

		///instantiates a bin of given size from an assii/ascii file. 
		/// @param : defines the filepath to load rectangles. 
		...
		/// Loads multiple rectangles into the bin
	...
	private:
		...
		///list of rectangles to be packed stored in a vector 
		std::vector<Rect> listRec;
